...
export const cleanAllSeed = asyncHandler(async (req, res) => {
  ...
  await Notice.create({
    ...
    message:
      'Your enterprise-grade document and operational management system is now live. Explore your dashboard!',
    type: 'success',
    createdBy: admin._id,
  });
  ...
  if (seedFiles.length > 0) {
    for (let i = 0; i < 20; i++) {
      ...
      fs.copyFileSync(
        ...
        path.join(destDir, uniqueName)
      );

      await Document.create({
        title: `Seeded Doc #${i + 1}`,
        description: `Statically retrieved from ${fileName}`,
        cloudUrl: `http://localhost:${process.env.PORT}/uploads/${uniqueName}`,
...
export const safeSeed = asyncHandler(async (req, res) => {
  ...
  if (users.length === 0)
    return res
      .status(400)
      .json(new ApiResponse(400, null, 'No users to attach docs to.'));
  ...
  return res
    .status(200)
    .json(
      new ApiResponse(200, { docsAppended }, 'Safe seed completed successfully')
    );
});
