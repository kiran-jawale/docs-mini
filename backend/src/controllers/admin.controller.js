...
export const deleteUser = asyncHandler(async (req, res) => {
  ...
  return res
    .status(200)
    .json(new ApiResponse(200, {}, 'User securely deleted.'));
});
...
export const getEmployeeById = asyncHandler(async (req, res) => {
  ...
  return res
    .status(200)
    .json(new ApiResponse(200, employee, 'Employee details retrieved.'));
});

export const updateEmployee = asyncHandler(async (req, res) => {
  const updatedEmployee = await adminService.updateEmployee(
    req.params.id,
    req.body,
    req.user.role,
    req.user._id
