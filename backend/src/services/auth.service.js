...
class AuthService {
  async register(data) {
    ...
    if (secretCode) {
      ...
      else throw new ApiError(400, UX_ERRORS.AUTH.INVALID_SECRET);
  ...
  async updateProfile(id, isEmployee, data) {
    ...
    if (!updated) throw new ApiError(404, UX_ERRORS.CRUD.NOT_FOUND);
    return updated;
  }
  ...
  async updateEmail(id, isEmployee, newEmail) {
    if (
      (await Employee.findOne({ email: newEmail })) ||
  ...
  async updateUserID(id, isEmployee, newUserID) {
    if (
      (await Employee.findOne({ userID: newUserID })) ||
      (await User.findOne({ userID: newUserID }))
    ) {
      throw new ApiError(409, UX_ERRORS.AUTH.ALREADY_EXISTS);
    }
    const Model = isEmployee ? Employee : User;
