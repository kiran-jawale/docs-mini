...
class AdminService {
  ...
  async updateEmployee(id, updateData, requesterRole, requesterId) {
    ...
    if (
      updateData.status &&
      !['active', 'disabled'].includes(updateData.status)
    ) {
      throw new ApiError(400, 'Invalid employee status value.');
    }
  ...
  async deleteAccount(targetId, targetType, requesterRole, requesterId) {
    ...
    if (targetType === 'employee') {
      ...
      if (
        requesterRole === 'hr' &&
        (targetRole === 'admin' || targetRole === 'hr')
      ) {
        throw new ApiError(403, UX_ERRORS.CRUD.HIERARCHY_VIOLATION);
      }
  ...
  async updateAccountStatus(
    ...
    if (targetType === 'employee') {
      ...
      if (
        !['active', 'restricted', 'blocked', 'disabled'].includes(newStatus)
      ) {
        throw new ApiError(400, 'Invalid user status value.');
      }
    }
