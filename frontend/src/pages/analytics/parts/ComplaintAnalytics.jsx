...
const ComplaintAnalytics = () => {
  ...
  const fetchData = async () => {
    setLoading(true);
    try {
      const res = await analyticsService.getComplaintAnalytics({
        userStatus: filter,
      });
  ...
  const totalTickets = data.statusCounts.reduce(
  ...
  return (
    <div className="space-y-6">
      ...
      {loading ? (
        <div className="p-10 animate-pulse text-xs font-black uppercase text-zinc-500">
          Mapping Helpdesk...
        </div>
      ) : (
        <>
          <div className="grid md:grid-cols-2 gap-6">
            ...
            <div className="bg-green-600 text-white p-6 rounded-2xl shadow-xl">
              ...
              <p className="text-4xl font-black mt-2">
                {Math.round(
                  ((data.statusCounts.find((s) => s._id === "resolved")
                    ?.count || 0) /
                    totalTickets) *
                    100
                ) || 0}
