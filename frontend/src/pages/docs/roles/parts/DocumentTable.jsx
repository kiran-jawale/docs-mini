...
const DocumentTable = ({ docs, refresh, isAdmin = false }) => {
  ...
  const handleDelete = async (id) => {
    if (!window.confirm("Permanently delete this document from the system?"))
      return;
    try {
        ...
        <tbody className="divide-y dark:divide-zinc-800">
          {docs.length === 0 ? (
            <tr>
              <td
                colSpan="4"
                className="p-10 text-center text-zinc-500 font-black text-xs uppercase tracking-widest"
              >
              ...
              >
                ...
                <td className="p-4">
                  <p className="text-xs font-bold dark:text-gray-300">
                    {d.owner?.fullname || "System Authored"}
                  </p>
