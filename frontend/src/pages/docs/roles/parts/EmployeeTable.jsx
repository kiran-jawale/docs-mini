...
const EmployeeTable = ({ employees, refresh }) => {
  ...
  const handleStatusToggle = async (id, currentStatus) => {
    ...
    try {
      ...
      addToast(
        `Employee access ${newStatus === "active" ? "restored" : "revoked"}`,
        "success"
      );
      refresh();
    } catch (error) {
        ...
        <tbody className="divide-y dark:divide-zinc-800">
          {employees.map((emp) => (
            ...
            >
              ...
              <td className="p-5">
                <span
                  className={`flex items-center gap-1.5 text-[10px] font-bold ${emp.status === "active" ? "text-green-500" : "text-red-500"}`}
                >
                  <span
                    className={`w-1.5 h-1.5 rounded-full ${emp.status === "active" ? "bg-green-500" : "bg-red-500"}`}
                  ></span>
              ...
              <td className="p-5 text-right">
                 {emp._id !== currentUser._id &&
                  (currentUser.role === "admin" || emp.role !== "hr") && (
                    <button
