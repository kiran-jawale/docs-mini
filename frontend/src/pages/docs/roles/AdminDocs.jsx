...
const AdminDocs = () => {
  ...
  const loadData = async () => {
    ...
    try {
      ...
      if (currentView === "#users") res = await adminService.getAllUsers();
        ...
        res = await adminService.getAllEmployees();
      else if (currentView === "#complaints")
        res = await complaintService.getAllComplaints();
      else if (currentView === "#notices")
        res = await noticeService.getNotices();
      else if (currentView === "#documents")
        res = await documentService.getPublicDocuments();
  ...
  return (
    <div className="p-8 pb-24">
      ...
      {loading ? (
        <div className="p-20 text-center text-zinc-500 animate-pulse font-black text-xs uppercase tracking-widest">
          Syncing Records...
        </div>
      ) : (
        <>
          ...
          {currentView === "#employees" && (
            <EmployeeTable employees={data} refresh={loadData} />
          )}
          {currentView === "#complaints" && (
            <ComplaintTable complaints={data} refresh={loadData} />
          )}
          {currentView === "#notices" && (
            <NoticeTable notices={data} refresh={loadData} />
          )}
          {currentView === "#documents" && (
            <DocumentTable docs={data} refresh={loadData} isAdmin={true} />
          )}
        </>
      )}
