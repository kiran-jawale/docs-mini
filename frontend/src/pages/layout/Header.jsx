...
const Header = () => {
  ...
  return (
    <header
      className={`${theme === "dark" ? "bg-zinc-900/80 text-white border-zinc-800" : "bg-white/80 text-gray-800 border-gray-100"} border-b backdrop-blur-md z-[60] sticky top-0 transition-all duration-300`}
    >
        ...
        <div className="hidden md:flex items-center space-x-6">
          {content.navLinks.map((link) => (
            <NavLink
              ...
              to={link.path}
              className={({ isActive }) =>
                `text-[10px] font-black uppercase tracking-widest ${isActive ? "text-green-500" : "text-zinc-400 hover:text-green-600"}`
              }
            >
              {link.title}
            </NavLink>
          ...
          {/* <button
            onClick={toggleTheme}
            className="p-2 rounded-xl hover:bg-zinc-50 dark:hover:bg-zinc-800/50 transition"
          >
            {theme === "light" ? "🌙" : "☀️"}
          </button> */}
              ...
              <div className="relative group py-2">
                <div className="flex items-center gap-3 cursor-pointer">
                  ...
                  <div className="hidden lg:block text-left">
                    ...
                    <p className="text-[9px] text-zinc-500 font-mono mt-1">
                      {user.userID || "ID UNSET"}
                    </p>
                  </div>
                  <span className="text-[8px] text-zinc-500 group-hover:rotate-180 transition-transform">
                    ▼
                  </span>
                </div>
                ...
                <div className="absolute right-0 top-full pt-2 w-52 opacity-0 translate-y-2 invisible group-hover:opacity-100 group-hover:translate-y-0 group-hover:visible transition-all duration-300 z-[70]">
                  <div className="bg-white dark:bg-zinc-800 rounded-2xl shadow-2xl border border-gray-100 dark:border-zinc-700 overflow-hidden p-2">
                    <Link
                      to="/profile"
                      className="flex items-center gap-3 px-4 py-3 text-xs font-bold hover:bg-green-50 dark:hover:bg-green-500/10 rounded-xl transition"
                    >
                      👤 My Profile
                    </Link>
                    <Link
